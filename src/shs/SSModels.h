...
class ssmodels_t
{
	public:
		...
		ssmodels_t(const int _xDim, const int uDim, const int dDim) noexcept : xDim
		{
			_xDim
		}
		, uDim
		...
		// modelialise state space model based on data from input Matlab file for IMDB based abstractions
															//TODO(ncauchi) replace all BMDP to IMDP
		void obtainIMDPfromMat(const char *fn, ssmodels_t &model, int currentMode);
