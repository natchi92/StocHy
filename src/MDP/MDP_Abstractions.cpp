...
void faust_t::MCapprox_Contr(double epsilon, shs_t<arma::mat, int> &model)
{
	...
	for (int i = 0; i < dim; i++)
	{
		...
		{
			X.col(i), arma::ones<arma::mat>(1, m),
	...
	for (int i = dim; i < 2 * dim; i++)
	{
		...
		std::vector<arma::mat> t1 =
		{
			arma::ones<arma::mat>(1, m), A,
			arma::ones<arma::mat>(1, q)
		};
		tempP = nDgrid_cb(t1);
		p.push_back(tempP);
	...
	std::vector<arma::mat> tem =
	{
		arma::ones<arma::mat>(m, 1),
...
void faust_t::formatOutput(double time, std::string cT, int Problem, int timeHorizon)
{
	...
	std::string str3("YES");
	exportOpt = "yes";
	std::cout << "Would you like to store the results generated by MDP [y- yes, n - no] " << std::endl;
	std::cin >> exportOpt;
	if ((exportOpt.compare(str) == 0) || (exportOpt.compare(str1) == 0)|| (exportOpt.compare(str2) == 0)|| (exportOpt.compare(str3) == 0))
	{
		...
		std::string f_name = "../results/MDP_Run_Time_" + cT + ".txt";
		myfile.open(f_name);
		myfile << time <<std::endl;
		myfile.close();

		// Storing value functions
		std::string Problem_solutionname = "../results/MDP_Problem_solution_"+ nS+ "_"  + cT + ".txt";
	...
	if ((exportOpt.compare(str) == 0) || (exportOpt.compare(str1) == 0) || (exportOpt.compare(str2) == 0)|| (exportOpt.compare(str3) == 0))
	{
		...
		if (dim == 2)
		{
			...
			oss << std::put_time(&tm, "%d-%m-%Y-%H-%M-%S");
			auto cT = oss.str();
			std::string f_name = "../results/MDP_gridPlot_" + std::to_string(nstates) + "_" + cT + ".py";
			myfile.open(f_name);
			...
			myfile << "cmap = plt.cm.jet(normal(c))\n\n";
			...
			myfile <<"plt.pcolor(X, Y, Z, cmap=plt.cm.jet, norm=normal)\n\n";
			...
			myfile << "plt.savefig(\"MDP_SatProbability.svg\",dpi=150)\n";
			myfile << "plt.show()\n";
